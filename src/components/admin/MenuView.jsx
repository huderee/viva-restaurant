...
import {
  ...
  Flame, Star, Tag, ImageOff, Trash2, Link
} from "lucide-react";
...
function ImagePreview({ url }) {
  ...
  if (!url) return (
    <div style={{
      ...
      <span style={{ fontSize: 11, color: 'var(--text-faint)' }}>Зургийн URL оруулна уу</span>
    </div>
  );

  return (
    <div style={{
      height: 160, borderRadius: 12, overflow: 'hidden', marginBottom: 16,
      border: `2px solid ${status === 'err' ? 'rgba(239,68,68,0.4)' : status === 'ok' ? 'rgba(16,185,129,0.4)' : 'var(--border)'}`,
      ...
      <img
        ...
        style={{ width: '100%', height: '100%', objectFit: 'cover', display: status === 'err' ? 'none' : 'block' }}
      />
      {status === 'err' && (
        <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
          <ImageOff size={22} color="#ef4444" />
          <span style={{ fontSize: 11, color: '#ef4444' }}>URL буруу эсвэл зураг олдсонгүй</span>
        </div>
      )}
      {status === 'ok' && (
        <>
          ...
          <div style={{ position: 'absolute', bottom: 8, left: 10, display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <span style={{ fontSize: 10, color: '#10b981', fontWeight: 700 }}>Зураг олдлоо</span>
          </div>
        </>
      )}
    </div>
    ...
    >
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '22px 24px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {/* Зургийн URL */}
          <div>
            <label style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 700, color: 'var(--text-faint)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.07em' }}>
              <Link size={11} /> Зургийн URL
            </label>
            <input
              className="input-field"
            ...
            <div style={{ fontSize: 10, color: 'var(--text-faint)', marginTop: 4 }}>
              Unsplash, Google зураг болон бусад зургийн URL-г ашиглаж болно
            </div>
          </div>
